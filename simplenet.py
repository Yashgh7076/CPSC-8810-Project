...
# Layer 2
W2 = tf.Variable(tf.truncated_normal(shape = [9, 9, depth_1, depth_2], stddev = 0.1))
...
W4 = tf.Variable(tf.truncated_normal(shape = [56 * 56 * depth_2, fc], stddev = 0.1))
...
Y2 = tf.nn.conv2d(Y1_out, W2, strides = [1,1,1,1], padding = 'SAME') + B2 # Image Size 112 x 112
Y2_max = tf.nn.max_pool(Y2, ksize = [1,2,2,1], strides = [1,2,2,1], padding = 'SAME')
Y2_out = tf.nn.relu(Y2_max) # Image Size 56 x 56

YY = tf.reshape(Y2_out, shape = [-1, 56 * 56 * depth_2])
Y4 = tf.nn.relu(tf.matmul(YY, W4) + B4)

Y4_drop = tf.nn.dropout(Y4, keep_prob = pkeep)

Y_logits= tf.matmul(Y4_drop, W5) + B5
...
batch_size = 32 # 32 // Change code to check if code is in train/test
