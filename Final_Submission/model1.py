...
for epoch in range(epochs):
    ...
    D_temp = D[indices, :, :, :]
    labels_temp = labels[indices]
    
    # Split the data into train & test for each epoch
    index = math.floor(0.67*(L9 + number))
    train_data = D_temp[0:index, :, :, :]
    train_labels = labels_temp[0:index]
    
    test_data = D_temp[index:L9 + number, :, :, :]
    test_labels = labels_temp[index:L9 + number]
