...
l10 = len(a10)
#number = max(l1, l2, l3, l4, l5, l6, l7, l8, l9)
number = 200
L10 = L9 + number
...
Y1_raf = tf.nn.relu(Y1_rbn) # No maxpooling within a resnet block
...
Y3_raf = tf.nn.relu(Y3_rbn) # No maxpooling within a resnet block
...
Y5_raf = tf.nn.relu(Y5_rbn) # No maxpooling within a resnet block
...
Y7_raf = tf.nn.relu(Y7_rbn) # No maxpooling within a resnet block
...
Y9_raf = tf.nn.relu(Y9_rbn) # No maxpooling within a resnet block
...
for epoch in range(epochs):
    ...
    for i in range(0, index, batch_size):
        ...
        sess.run(train_step, feed_dict = {X: batch_X, Y: batch_Y, training: True, pkeep: 0.2, pkeep_conv: 0.7, lr: learn_rate})
        a,c = sess.run([accuracy,cross_entropy], feed_dict = {X: batch_X, Y: batch_Y, pkeep: 1.0, pkeep_conv: 1.0, lr: learn_rate})
        train_acc = a
        train_loss = c   

        # Save model after each iteration
        save_path = saver.save(sess,"CPSC_8810/model_12/model_resnet")
...
f.close()        
np.savez('CPSC_8810/plots/model_12resnet_4_128.npz',name1 = batch_acc, name2 = batch_loss, name3 = test_acc, name4 = test_loss, name5 = epochs)
